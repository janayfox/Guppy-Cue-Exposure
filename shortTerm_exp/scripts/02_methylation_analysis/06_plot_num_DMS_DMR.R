...
#plot upsets 
plot.upset.v2 <- function(data){ 
  upset(data, sets = c("0.5h", "1h", "4h", "24h", "72h"), order.by = "freq", keep.order = TRUE, 
        point.size = 2.5, line.size = 1, text.scale = c(1.5, 1.5, 1.3, 1.3, 2, 1.15), main.bar.color = "#39568CFF",
        matrix.color = "#1F968BFF", sets.bar.color = "#39568CFF")
  }

DMS.fem.upset.new.plot <- plot.upset.v2(DMS.fem.upset.new)
DMS.mal.upset.new.plot <- plot.upset.v2(DMS.mal.upset.new)
DMS.all.upset.new.plot <- plot.upset.v2(DMS.all.upset.new)

DMR.fem.upset.new.plot <- plot.upset.v2(DMR.fem.upset.new)
DMR.mal.upset.new.plot <- plot.upset.v2(DMR.mal.upset.new)
...
mod1_fem <- Mclust(fem_data, G = 1, family )
mod2_fem <- Mclust(fem_data, G = 2)

# Fit models for males
mod1_mal <- Mclust(mal_data, G = 1)
mod2_mal <- Mclust(mal_data, G = 2)
...
#fit log likelihood ratio test 
logLik(mod1_fem) - logLik(mod2_fem)
1-pchisq(-8.041613, df=5)


fem_poisson_mod <- flexmix(num_DMS ~ time_point_numeric, data = fem_data, k = 2, model = FLXMRglm(family = "poisson"))
summary(fem_poisson_mod)
mal_poisson_mod <- flexmix(num_DMS ~ time_point_numeric, data = mal_data, k = 2, model = FLXMRglm(family = "poisson"))
summary(mal_poisson_mod)

# Model comparison using BIC
print("BIC for females:")
print(BIC(fem_poisson_mod))
print("BIC for males:")
print(BIC(mal_poisson_mod))

#try diptest
dip.test(fem_data$num_DMS, simulate.p.value = TRUE, B =1000)
dip.test(mal_data$num_DMS, simulate.p.value = TRUE, B =1000)

#try silverman test
silverman.test(fem_data$num_DMS, k = 2, R = 100)
silverman.test(mal_data$num_DMS, k = 2, R = 100)
